...
**The gap between *permitted* and *exposed* is latent capability:** authorised, not currently offered, and held back by nothing the deployer controls. Attachment content is the worked example. It belongs in the grant as a row that is *permitted, not exposed*, not in a list called *not reachable*.
...
1. **Does *grant* stay the name for storey two, or move to storey one?** The memo calls the OAuth layer *"the grant"*. Recommendation: **the grant stays what the agent can do today (storey two), and gains storeys as properties on each row.** The delta is computed against the grant, and if the grant becomes the ceiling then every policy acquires a large unbounded excess that is not true today, which would be the manufactured assurance rule in reverse. *Permitted* is a property of a row, not a replacement for it.
