...
| `docs/briefs/architecture__the-stories-vault-and-the-three-way-workflow.md` | 26 Sept. The stories vault: the lead, the studio model and this site's agent as publisher, one sgit vault in the Email-FS-lite protocol, the seed under `stories-vault/seed/`, the board derived from issues and mailrooms, the birth in commands, the scheduled check-in and what it may not decide | the stories, the vault, a check-in, the board |
| `docs/briefs/merch__funny-designs-for-people-who-hold-the-risk.md` | 25 Sept. A brief for an image model: shirts a stranger buys; the brand kit, the print rules, forty-two taglines in six families with the joke explained, twelve concepts, the prompt to paste; handed over by its console twin's link | making merchandise, or any use of the mark off the site |
...
| The team | `site/team/` (in *Behaviour policies*): `index.html`, `publisher.html`, `designer.html`, their twins, and the data `team.json`, `publisher.json`, `designer.json` | **generated** by `build-team.mjs`. Our own agents' ABPs, the three tiers (public, private, secret), the surfaces, the workflow from private to public, the controls and tooling in place and not. The publisher's role for a clean session is `.claude/agents/publisher.md`; the check-in prompt is `.claude/commands/publisher-check-in.md` |
| Stories | `site/stories/` (in *Reading*, with Articles): `index.html`, `<slug>.html`, their twins, `images/`, and the data `cast.json`, `<slug>.json`, `board.json` | **generated** by `build-stories.mjs` from the JSON; the old flat addresses (`stories.html`, `story-*.html`) are redirect stubs. Brief D29; `architecture__the-stories-vault-…` |
...
| `scripts/site/build-stories.mjs [--check]` | the stories: `site/stories/cast.json`, one JSON per story and `board.json` → `site/stories/index.html` and `site/stories/<slug>.html`, root-absolute links, `RM.data.base="/"`; refuses an unknown cast member or the ladder word this site never uses | yes | yes |
| `scripts/site/build-team.mjs [--check]` | the team: `site/team/team.json` (people, tiers, surfaces, workflow, tooling) and `site/team/<agent>.json` (an Agent Behaviour Policy each, in the vault grammar) → `site/team/index.html` and `site/team/<agent>.html`; derives each delta as the vaults do; refuses a row without barrier, evidence, undo and note, a mandate that wants and refuses the same row, the two banned words and anything credential-shaped | yes | yes |
| `scripts/stories/mail.mjs <command> --vault <clone>` | Email-FS-lite for the stories vault, no dependencies: `init`, `send`, `deliver`, `done`, `issue open\|block\|unblock\|close`, `board` (derives `board/board.json`, re-inlines it in `board/index.html`, `--site` writes `site/stories/board.json`), `status`. Nothing in it touches the network | yes | no |
...
| The stories vault: the lead, `studio.chatgpt`, `publisher.claude` (us), Email-FS-lite; not born as at 26 Sept; its seed is `stories-vault/seed/` | sgit, key and token from the lead in a session | yes, our own folder and the mirrored `stories/` |
