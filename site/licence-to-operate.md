<!-- Generated from licence-to-operate.html by scripts/site/generate.mjs. Edit the page, not this file. -->
...
- **Authority**The organisation, through a named accountable owner. Not a team and not a function — accountability that cannot be forwarded
- **Licensee**One agent in one deployment, identified by its shape and the version of the grant it was licensed against
- **Instrument**The behaviour policy itself: the mandate, the delta and the vocabulary, each pinned to a version
- **Valid until**An interval, set when it is issued. A licence with no expiry is not a decision
- **Scope**Each condition beside the thing that actually enforces it — or beside the admission that nothing does

The last row is the one that does the work. A condition with nothing enforcing it is an expectation, and [three of the four barrier kinds bound nothing at all](questions.html#words). A licence that does not distinguish them is a document that reads like a control and is not one.
