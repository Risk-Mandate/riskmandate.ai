...
On a phone, open it in its own window: the frame below is the same app, sandboxed, and small. The vault browser is the whole product — files, history and the app — and the button carries the public read key, so it opens read-only without a paste. The copy this site serves at [vaults/claude-m365-connector/index.html](vaults/claude-m365-connector/index.html) says in its top bar which route it loaded the app by.
