...
| | |
| --- | --- |
| **Licensee** | Claude, with the Microsoft 365 connector: Outlook, SharePoint, OneDrive, Teams — deployment shape `anthropic/microsoft-365-connector/default`, grant version 2026-09-15 |
| **Authority** | — not yet issued to anyone — |
| **Accountable owner** | — unassigned — |
| **Instrument** | This behaviour policy: mandate `search-our-tenant-read-only` (2026-09-15), delta `anthropic__microsoft-365-connector__default__search-our-tenant-read-only` (2026-09-15T02:43:02Z), vocabulary abp.sgit.ai v0.3.0 |
...
| --- | --- | --- | --- |
| Do not `send.message.world` (send a message to anyone) | the mandate refuses it | ○ boundary | Mail.Send, delegated; "Revoke … in Entra" is the off switch, and it is the administrator's, not the user's |
| Do not `authenticate-as.credential.tenant` (act in accounts with the credentials it holds) | the mandate never authorised it | ○ boundary | delegated permissions; "Multi-tenant isolation is cryptographically enforced through digitally signed access tokens" |
