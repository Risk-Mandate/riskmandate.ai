...
**Mandate id** `reader-on-my-drive` · **status** starting-point · **authored** 2026-09-15 by the site, as a starting point — not measured, not surveyed; the first thing to argue with
...
| --- | --- | --- |
| `read.file.host` | Read any file the account can reach | host — the Drive as a store: every file owned by or shared to the user |

## What is explicitly not wanted (3)

| Capability | What it is | Reach |
| --- | --- | --- |
| `write.file.host` | Change any file the account can reach | host — the Drive as a store: every file owned by or shared to the user |
| `delete.file.host` | Delete files anywhere the account can reach | host — the Drive as a store: every file owned by or shared to the user |
| `read.credential.host` | Read credentials stored where it runs | host — the Drive as a store: every file owned by or shared to the user |
