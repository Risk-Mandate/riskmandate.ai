# An assistant on a personal Google Drive — an Agent Behaviour Policy, as a vault
...
| `RESEARCH-NEEDED.md` | 3 questions the grant cannot settle from published pages, each with how to settle it — hand them to an agent | derived from the grant; answered by whoever researches |
...
```
node scripts/site/build-abp-vault.mjs google-drive-readonly
```
