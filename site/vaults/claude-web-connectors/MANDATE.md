...
**Mandate id** `chat-with-connectors` · **status** starting-point · **authored** 2026-09-09 by the site, as a starting point — not measured, not surveyed; the first thing to argue with
...
| `read.file.host` | Read any file the account can reach | host — what the drive connector is scoped to; not your machine |
| `read.message.tenant` | Read mail or chat it is connected to | tenant — the accounts you connected, as you scoped them |

## What is explicitly not wanted (2)

| Capability | What it is | Reach |
| --- | --- | --- |
| `send.message.world` | Send a message to anyone | world — the vendor's egress |
| `write.repository.tenant` | Push to a code host (any branch it can reach) | tenant — the accounts you connected, as you scoped them |
...
| `authenticate-as.credential.tenant` | Act in accounts with the credentials it holds | yes | unstated: it is how a connector works; the question is what it is used for, which is the two rows below it |
