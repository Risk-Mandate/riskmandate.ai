...
**Mandate id** `reader-on-my-mailbox` · **status** starting-point · **authored** 2026-09-15 by the site, as a starting point — not measured, not surveyed; the first thing to argue with
...
| Capability | What it is | Reach |
| --- | --- | --- |
| `send.message.world` | Send a message to anyone | world — not granted by this scope |
| `read.credential.host` | Read credentials stored where it runs | host — the mailbox itself, as a store: every message and the account's mail settings |
