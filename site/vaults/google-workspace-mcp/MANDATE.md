...
**Mandate id** `assistant-over-my-workspace` · **status** starting-point · **authored** 2026-09-15 by the site, as a starting point — not measured, not surveyed; the first thing to argue with
...
| --- | --- | --- |
| `read.message.tenant` | Read mail or chat it is connected to | tenant — the Google account the consent was given for, and the Workspace domain it belongs to |
| `read.file.host` | Read any file the account can reach | host — the Google account's Drive and mailbox — every file owned by or shared to the user; not your machine |

## What is explicitly not wanted (3)

| Capability | What it is | Reach |
| --- | --- | --- |
| `send.message.world` | Send a message to anyone | world — anyone reachable by mail from that account |
| `write.file.host` | Change any file the account can reach | host — the Google account's Drive and mailbox — every file owned by or shared to the user; not your machine |
| `read.credential.host` | Read credentials stored where it runs | host — the Google account's Drive and mailbox — every file owned by or shared to the user; not your machine |
