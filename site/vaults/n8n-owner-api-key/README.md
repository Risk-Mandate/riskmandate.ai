# A self-hosted n8n instance, owner API key — an Agent Behaviour Policy, as a vault
...
| `RESEARCH-NEEDED.md` | 4 questions the grant cannot settle from published pages, each with how to settle it — hand them to an agent | derived from the grant; answered by whoever researches |
...
```
node scripts/site/build-abp-vault.mjs n8n-owner-api-key
```
