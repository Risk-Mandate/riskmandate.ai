...
| | |
| --- | --- |
| **Licensee** | Claude Desktop (a desktop app with local tools) — deployment shape `anthropic/claude-desktop/default`, grant version 2026-09-05 |
| **Authority** | — not yet issued to anyone — |
| **Accountable owner** | — unassigned — |
| **Instrument** | This behaviour policy: mandate `desktop-app-with-local-tools` (2026-09-09), delta `anthropic__claude-desktop__default__desktop-app-with-local-tools` (2026-09-15T01:41:06Z), vocabulary abp.sgit.ai v0.3.0 |
...
## Conditions — what the licensee is asked not to do, and what enforces each (8)
...
| Do not `read.file.host` (read any file the account can reach) | the mandate never authorised it | ◐ setting | the app's folder permission |
| Do not `execute.process.host` (run programs as the account) | the mandate refuses it | ◐ setting | a confirmation prompt |
| Do not `write.file.host` (change any file the account can reach) | the mandate refuses it | ◐ setting | the app's folder permission |
| Do not `grant.credential.self` (change its own permission settings) | the mandate refuses it | ◐ setting | the app's own settings file |
