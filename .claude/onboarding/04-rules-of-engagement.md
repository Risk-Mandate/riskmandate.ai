...
| **Generated files** (`*.md` twins, `sitemap.xml`, `llms.txt`, `llms-full.txt`, `404.html`, `versions.md`, `abp-vault-*.html`, `agent-behaviour-policy.html`, everything derived inside `site/vaults/<slug>/`) | both sides regenerate | Never resolve by hand. Take either side, then run the generator (`generate.mjs`, `build-abp-pages.mjs`, `build-abp-vault.mjs <slug>`) and commit its output |
