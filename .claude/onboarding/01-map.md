...
| `scripts/site/build-stories.mjs [--check]` | the stories: `site/stories/cast.json` and one JSON per story → `stories.html` and `story-<slug>.html`; refuses an unknown cast member or the ladder word this site never uses | yes | yes |
