# Claude's Gmail connector, for Example Analytics — an Agent Behaviour Policy, as a vault

> This is one deployment, described. Everything in it is derived from three inputs, and one of them is yours.
...
| `data/scenarios.json` | 8 scenarios — alternative mandates for the same grant (3 normal use, 5 advanced); the delta is recomputed per scenario | written here as starting points; the grant never changes |
| `RESEARCH-NEEDED.md` | 6 questions the grant cannot settle from published pages, each with how to settle it — hand them to an agent | derived from the grant; answered by whoever researches |
...
```
node scripts/site/build-abp-vault.mjs example-analytics
```
