# AGENT BEHAVIOUR POLICY — Claude on one Gmail mailbox — a customer's draft
...
| The delta | Excess and shortfall | **Derived** — recomputed whenever either input moves, never edited | `DELTA.md` — 4 excess, 3 unbounded, 0 shortfall |
...
| `send.message.world` | Send a message to anyone | ◐ setting | no | measured ✓ | in the mandate |
...
**4 in the grant that the mandate did not ask for.** 2 of those it refused; 2 it never mentioned. **3 have nothing but a setting, a sentence or nothing at all in the way.** Nothing wanted is missing.
